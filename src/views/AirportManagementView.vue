...
// Desestructurar composable de aeropuertos
const {
  airports,
  loading,
  error,
...
// Filtrado de aeropuertos en cliente (búsqueda)
const filteredAirports = computed(() => {
  if (!searchQuery.value) return airports.value
  const query = searchQuery.value.toLowerCase().trim()
  return airports.value.filter(ap => 
    (ap.nombre || '').toLowerCase().includes(query) || 
    (ap.codigo || '').toLowerCase().includes(query) ||
    (ap.pais || '').toLowerCase().includes(query)
  )
})
...
const handleUpdateAirport = async () => {
  ...
  try {
    ...
    await updateAirport(selectedAirport.value.id, command)
  ...
  <div class="page-management container">
    ...
    <header class="management-header">
      <div>
        ...
        <p class="page-subtitle">Catálogo global de aeropuertos y terminales aéreas en el sistema.</p>
      </div>
      <button 
    ...
    <div v-if="authStore.user?.role !== 'Administrador'" class="role-warning-banner glass-card">
      ...
      <div class="warning-body">
        ...
        <p>Tu rol actual es <strong>{{ authStore.user?.role || 'Desconocido' }}</strong>. Las operaciones de registro, edición y eliminación de aeropuertos están reservadas exclusivamente para usuarios con rol de <strong>Administrador</strong>.</p>
      </div>
    </div>
    ...
    <div class="toolbar-section glass-card" style="margin-bottom: 1.5rem; padding: 1rem 1.5rem;">
      <div class="search-box-wrapper">
        ...
        <input 
          v-model="searchQuery" 
          type="text" 
          placeholder="Buscar aeropuerto por nombre, código IATA o país..." 
    ...
    <div class="management-card glass-card">
      ...
      <div v-if="loading" class="loader-container">
        ...
        <p>Actualizando catálogo de aeropuertos...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-container">
        <p class="error-text">{{ error }}</p>
        <button @click="fetchAirports" class="btn btn-primary">Reintentar</button>
      </div>
        ...
        <table class="flights-table">
          ...
          <tbody>
            <tr v-for="airport in filteredAirports" :key="airport.id">
              <td class="font-bold text-primary-color">{{ airport.codigo }}</td>
              <td class="font-semibold text-white">{{ airport.nombre }}</td>
              <td>{{ airport.pais }}</td>
              ...
              <td v-if="authStore.user?.role === 'Administrador'" class="text-right actions-cell">
                <div class="action-buttons-group">
                  <button @click="openEditModal(airport)" class="btn-action btn-edit" title="Editar aeropuerto">
                    Editar
                  </button>
                  <button @click="openDeleteModal(airport)" class="btn-action btn-cancel" title="Eliminar aeropuerto">
                    Eliminar
                  </button>
                </div>
              </td>
            ...
            <tr v-if="filteredAirports.length === 0">
              <td colspan="5" class="empty-table-cell">
                No se encontraron aeropuertos que coincidan con la búsqueda.
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <footer class="table-footer">
        <span class="pagination-info">Mostrando {{ filteredAirports.length }} aeropuertos registrados</span>
      </footer>
    </div>

    <!-- Modals Layer (Teleport) -->
    <Teleport to="body">
      <!-- 1. Registrar / Crear Aeropuerto -->
      <div v-if="activeModal === 'create'" class="modal-backdrop" @click="closeModal">
        <div class="modal-container glass-card" @click.stop>
          ...
          <h2 class="modal-title">Agregar Aeropuerto</h2>
          <p class="modal-subtitle">Ingrese los datos para registrar una nueva terminal aérea en el catálogo.</p>
          
          <div class="form-fields">
            <div class="form-group">
              <label class="form-label" for="ap-code">Código IATA</label>
              <input id="ap-code" v-model="form.codigo" type="text" class="form-input" placeholder="Ej. SDQ, JFK, MIA" maxlength="5" required />
            </div>

            <div class="form-group">
              <label class="form-label" for="ap-name">Nombre de Aeropuerto</label>
              <input id="ap-name" v-model="form.nombre" type="text" class="form-input" placeholder="Ej. Las Américas International Airport" required />
            </div>
          ...
          <div class="modal-footer">
            ...
            <button @click="handleCreateAirport" class="btn btn-primary" :disabled="loadingOperation">
              {{ loadingOperation ? 'Registrando...' : 'Registrar Aeropuerto' }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeModal === 'edit'" class="modal-backdrop" @click="closeModal">
        <div class="modal-container glass-card" @click.stop>
          <button class="modal-close" @click="closeModal">&times;</button>
          <h2 class="modal-title">Editar Aeropuerto</h2>
          <p class="modal-subtitle">Modifique los datos operativos para la terminal aérea seleccionada.</p>
          
          <div class="form-fields">
            <div class="form-group">
              <label class="form-label" for="eap-code">Código IATA</label>
              <input id="eap-code" v-model="form.codigo" type="text" class="form-input" placeholder="Ej. SDQ, JFK, MIA" maxlength="5" required />
            </div>
          ...
          <div class="modal-footer">
            <button @click="closeModal" class="btn btn-secondary" :disabled="loadingOperation">Cancelar</button>
            <button @click="handleUpdateAirport" class="btn btn-primary" :disabled="loadingOperation">
